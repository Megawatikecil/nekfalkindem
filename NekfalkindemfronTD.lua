...
function a1()
    ...
    if choice == 1 then
        ...
        gg.clearList()
        gg.clearResults()
        gg.toast("✅ H4ck Activate, value = "..silver_input[1])
...
function itemid()
    ...
    if not x102_input then
        gg.toast("❌ Batal input")
        return
    end

    local xinput_val = tonumber(x102_input[1])
    if not xinput_val then
        gg.toast("❌ Input tidak valid")
        return
    end
    gg.toast("✅ Value diset: " .. xinput_val)
    ...
    if #x102list == 0 then
        ...
        if x102 and #x102 > 0 then
            ...
            gg.toast("❌ value tidak ditemukan")
            return
        end
    else
        gg.toast("♻️ value diambil dari List")
    end
    ...
    gg.toast("✅ value diubah jadi " .. xinput_val)
